<template>
  <div class="max-w-6xl mx-auto p-8">
    ...
    <form @submit.prevent="Submit" class="bg-white p-6 rounded-xl shadow mb-10 grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <input v-model="form.color" placeholder="Rangi" class="border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        <img :src="item.img" alt="Car" class="w-full h-48 object-cover" />
        <div class="p-4 space-y-1">
          <h2 class="text-lg font-bold text-gray-800"> {{ item.color }}</h2>
          <p class="text-sm text-gray-600">Yil: {{ item.year }}</p>
